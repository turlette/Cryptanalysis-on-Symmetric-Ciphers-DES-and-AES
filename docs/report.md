...
### 2.5. Hạn chế & Thực nghiệm Mở rộng (Experiment E: Fault Injection)
- **Đánh giá rủi ro phần cứng (DFA):** Tấn công chích lỗi (Glitching/Fault Injection) bằng việc sử dụng tia laser hoặc làm sụt áp ép phần cứng tính toán sai kết quả ở các vòng AES cuối cùng, từ đó dùng kỹ thuật Differential Fault Analysis để đảo ngược Khóa. Tuy nhiên, do rủi ro vật lý cực cao (có thể làm hỏng chip/bricking) và yêu cầu thiết bị đắt tiền, thực nghiệm này được liệt kê vào nhóm "vượt giới hạn an toàn Lab" và chỉ nghiên cứu ở mức độ lý thuyết. 
- **Khuyến nghị:** Thiết kế IoT cần có cơ chế phát hiện sụt áp, mã hóa chống lỗi (Redundancy) và tự xóa khóa (Tamper Resistance).
...
1. **Biham, E., & Shamir, A. (1991).** *Differential Cryptanalysis of DES-like Cryptosystems*. Journal of Cryptology.
2. **Matsui, M. (1993).** *Linear Cryptanalysis Method for DES Cipher*. EUROCRYPT '93.
3. **Bhargavan, K., & Leurent, G. (2016).** *On the Practical (In-)Security of 64-bit Block Ciphers: Collision Attacks on HTTP over TLS and OpenVPN (Sweet32)*. ACM CCS.
4. **Vaudenay, S. (2002).** *Security Flaws Induced by CBC Padding*. EUROCRYPT '02.
5. **Bernstein, D. J. (2005).** *Cache-timing attacks on AES*.
6. **Al Fardan, N. J., & Paterson, K. G. (2013).** *Lucky Thirteen: Breaking the TLS and DTLS Record Protocols*. IEEE S&P.
7. **Böck, H., Zauner, A., Devigne, S., et al. (2016).** *Nonce-Disrespecting Adversaries: Practical Forgery Attacks on GCM in TLS*. WOOT '16.

### Công cụ và Mã nguồn (Tools & Frameworks)
1. **OpenSSL:** Thư viện mật mã tham chiếu cho các bản vá lỗi (Mitigation).
2. **Hashcat:** Công cụ tấn công vét cạn bằng sức mạnh GPU.
3. **PadBuster:** Công cụ tự động hóa việc khai thác Padding Oracle trên Web/API.
4. **ChipWhisperer:** Hệ sinh thái phần cứng phân tích Side-channel và Fault Injection.